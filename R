...
df.bribe = read.csv(file="I paid a bribe.csv", header=TRUE, sep=",")
...
## Plot the amount of reports of each day ##

df.bribe.report = group_by(df.bribe, Date)
detach("package:plyr", unload=TRUE) 
df.bribe.report = summarise(df.bribe.report, count = n())
df.bribe.report$newcount = df.bribe.report$count*50
df.bribe.report$Date = as.Date(df.bribe.report$Date, format = "%B %d, %Y")
df.bribe.report$day = weekdays(df.bribe.report$Date)
df.bribe.report = df.bribe.report[-c(24),]

h = (max(as.numeric(df.bribe.report$count)) - min(as.numeric(df.bribe.report$count)) / sqrt(dim(df.bribe.report)[1]))

p = ggplot(df.bribe.report, aes(x = Date, y = count))
p +  geom_bar(color = "black", bindwith = h, stat = "identity") +  geom_text(aes(label=day), position=position_dodge(width=0.9), vjust=-0.25, size = 3.5) +
  labs(title = "Views of the last 1000 reports for each day compared to the reported Amount", x = "", y = "Views") + 
  theme(plot.title = element_text(lineheight=.5, face="bold"))
